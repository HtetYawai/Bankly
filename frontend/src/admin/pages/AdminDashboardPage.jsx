        ...
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Section title="Recently registered users" subtitle="Newest customer accounts" action={<Link to="/admin/users" className="text-xs font-semibold text-cyan-700 hover:underline">View users</Link>}>
              {!recentUsers.length ? <div className="p-8 text-center text-sm text-slate-500">No registered users are available.</div> : <ul className="divide-y divide-slate-100">{recentUsers.slice(0, 6).map((user) => <li key={user._id} className="flex items-center gap-3 px-5 py-3"><div className="grid h-9 w-9 place-items-center rounded-full bg-slate-100 text-sm font-bold text-slate-600">{user.fullName?.charAt(0)?.toUpperCase() ?? "U"}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-800">{user.fullName}</p><p className="truncate text-xs text-slate-500">{user.email}</p></div><div className="text-right"><span className="badge badge-ghost badge-sm">{user.accountStatus}</span><p className="mt-1 text-xs text-slate-400">{shortDate.format(new Date(user.createdAt))}</p></div></li>)}</ul>}
            </Section>
